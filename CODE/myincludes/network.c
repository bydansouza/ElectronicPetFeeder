...
void init_mqtt_topics(){
	...
	if (!wifi_my_host_name){
		...
		for (int i=0; i < CFG_TOPICS; i++) {
			snprintf(cfg_topics[i], PUB_TPC_LEN, "%s/cfg/%d", wifi_my_host_name,i);		//eor-b27071/cgf/0..1..2..3
			debug("%s\n", cfg_topics[i]);
		}
	}
...
void  mqtt_task(void *pvParameters)
{
    ...
    while(1) {
		...
		sysparam_set_string("wifi_mqtt_ip_addr", "172.16.0.2");			//Inserted in 01/12/2018@15:00
